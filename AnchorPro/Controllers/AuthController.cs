...
/// <summary>
        ...
        private async Task<List<string>> GetUserAllowedRoutesAsync(int? tenantId, List<string> roles)
        {
            ...
            foreach (var role in roles)
            {
                ...
                {
                    // Fallback defaults for a role with no TenantRolePermission row yet (e.g. right
                    // after registration, before anyone has visited Roles & Permissions).
                    var defaultRoutes = AnchorPro.Services.DefaultRolePermissions.GetDefaultRoutes(role);
